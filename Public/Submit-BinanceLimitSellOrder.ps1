#--------------------------------------------------------------------------------#
# Submit-BinanceLimitSellOrder                                                   #
#--------------------------------------------------------------------------------#
function Submit-BinanceLimitSellOrder
{
    param(
    ...
    if (-not([string]::IsNullOrEmpty($Quantity)))
    {
        ...
        $QueryString = Get-BinanceSignedQueryString -Param1 "symbol=$Market" -Param2 "side=SELL" -Param3 "type=LIMIT" -Param4 "timeInForce=GTC" -Param5 "quantity=$SellQty" -Param6 "price=$Price" -Param7 "recvWindow=5000"
    }
    ...
    Test-BinanceOrderParameters -Market $Market -Price $Price -Quantity $SellQty -Type "LIMIT"
